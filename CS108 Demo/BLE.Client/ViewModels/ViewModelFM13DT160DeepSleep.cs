...
{
    public class ViewModelFM13DT160DeepSleep : BaseViewModel
    {
        ...
        public ViewModelFM13DT160DeepSleep(IAdapter adapter, IUserDialogs userDialogs) : base(adapter)
        {
        ...
        void TagCompletedEvent(object sender, CSLibrary.Events.OnFM13DTAccessCompletedEventArgs e)
        {
            InvokeOnMainThread(() =>
            {
                if (e.access ==  CSLibrary.Constants.FM13DTAccess.DEEPSLEEP)
                {
                    if (e.success)
                    {
        ...
        void OnReadButtonButtonClick()
        {
            ...
            BleMvxApplication._reader.rfid.StartOperation(CSLibrary.Constants.Operation.FM13DT_DEEPSLEEP);
        }
