...
{
    public class ViewModelFM13DT160OpModeCheck : BaseViewModel
    {
        ...
        public ViewModelFM13DT160OpModeCheck(IAdapter adapter, IUserDialogs userDialogs) : base(adapter)
        {
        ...
        void TagCompletedEvent(object sender, CSLibrary.Events.OnFM13DTAccessCompletedEventArgs e)
        {
            InvokeOnMainThread(() =>
            {
                if (e.access ==  CSLibrary.Constants.FM13DTAccess.OPMODECHK)
                {
                    if (e.success)
                    {
        ...
        void OnReadButtonButtonClick()
        {
            ...
            BleMvxApplication._reader.rfid.Options.FM13DTOpModeChk.enable = switchEnableIsToggled;
            BleMvxApplication._reader.rfid.StartOperation(CSLibrary.Constants.Operation.FM13DT_OPMODECHK);
        }
